...
import cn.evolvefield.mods.atomlib.utils.TimeUtil;
...
public abstract class MorphItem {
    ...
    public CompoundTag serialize() {
        ...
        if (isDisabled()) {
            nbt.putInt("disabled_until", TimeUtil.convertToRelativeTime(disabledUntil));
    ...
    public void deserialize(CompoundTag nbt) {
        if (!nbt.getString("id").equals(getMorphItemId()))
            ...
            if (nbt.contains("disabled_until", Tag.TAG_INT)) {
                disabledUntil = TimeUtil.convertToAbsoluteTime(nbt.getInt("disabled_until"));
...
//    }
    ...
    public void disable(int forTicks) {
        ...
        this.disabledUntil = TimeUtil.convertToAbsoluteTime(forTicks);
    }

    public boolean isDisabled() {
        return TimeUtil.getUniversalTickTime() < disabledUntil;
    }

    // This calc might be wrong lol
    public float getDisabledProgress(float partialTicks) {
        float timeLeft = TimeUtil.convertToRelativeTime(disabledUntil);
